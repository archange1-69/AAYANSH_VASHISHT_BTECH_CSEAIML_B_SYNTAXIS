...
const { EModelEndpoint, Constants, openAISettings, CacheKeys } = require('syntaxis-data-provider');
...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing syntaxis conversation');
    return importsyntaxisConvo;
  }
...
/**
 * Imports a syntaxis conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importsyntaxisConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
) {
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid syntaxis file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from syntaxis file`, error);
  }
}
