...
(async () => {
  ...
  if (docker) {
    ...
    console.purple('Pruning all syntaxis Docker images...');

    const imageName = singleCompose ? 'syntaxis_single' : 'syntaxis';
    try {
      execSync(`${sudo}docker rmi ${imageName}:latest`, { stdio: 'inherit' });
    } catch (e) {
      console.purple('Failed to remove Docker image syntaxis:latest. It might not exist.');
    }
    console.purple('Removing all unused dangling Docker images...');
    execSync(`${sudo}docker image prune -f`, { stdio: 'inherit' });
    console.purple('Building new syntaxis image...');
  ...
  console.green('Your syntaxis app is now up to date! Start the app with the following command:');
  console.purple(startCommand);
  console.orange(
    'Note: it\'s also recommended to clear your browser cookies and localStorage for syntaxis to assure a fully clean installation.',
  );
