...
import { AuthType, Tools, QueryKeys } from 'syntaxis-data-provider';
import { useUpdateUserPluginsMutation } from 'syntaxis-data-provider/react-query';
...
const useAuthCodeTool = (options?: { isEntityTool: boolean }) => {
  ...
  const installTool = useCallback(
    (apiKey: string) => {
      updateUserPlugins.mutate({
        ...
        action: 'install',
        auth: { syntaxis_CODE_API_KEY: apiKey },
        isEntityTool,
      });
    },
  ...
  const removeTool = useCallback(() => {
    updateUserPlugins.mutate({
      pluginKey: Tools.execute_code,
      action: 'uninstall',
      auth: { syntaxis_CODE_API_KEY: null },
      isEntityTool,
    });
  }, [updateUserPlugins, isEntityTool]);
